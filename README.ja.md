...
- 追加: 複数テキスト音声合成のJSON入力に全オプション付きサンプルを表示
- 修正: JSON入力モードで文字列が正しくパースされない問題を修正

### 0.4.6

- 変更: 音声合成（簡略）の音声パラメータ（話速・音高・抑揚・音量・開始/終了無音）をオプション追加形式に変更
- 変更: 複数テキスト音声合成のベース設定の表示名に「ベース」を付加（ベース話者ID、ベース話速など）
- 削除: サンプリングレート設定を削除（APIデフォルト値を使用）

### 0.4.5

- 変更: 複数テキスト音声合成のベース音声設定（話速・音高・抑揚・音量・開始/終了無音）をオプション追加形式に変更
- 変更: テキスト毎の話者IDをオプション個別設定に移動（-1センチネル廃止）
- 変更: JSON入力モードで全オーバーライドフィールドに対応
- 改善: パラメータ優先順位を明確化（テキスト個別設定 > ベース設定 > APIデフォルト）
